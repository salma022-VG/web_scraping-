...
# ---------------------------------------------------------------------------
# Aqui esta la lista de temas (categorias) que interesan, y para cada uno,
# las frases que se buscan en Google Noticias. Si en el futuro quieres que
# el programa busque tambien otro tema, o cambiar una palabra de busqueda,
# ES AQUI donde se edita: solo agrega o quita frases entre comillas.
#
# Mientras mas frases de busqueda tenga un tema, mas noticias relacionadas
# va a poder encontrar (pero tambien puede traer alguna que no sea 100%
# relevante; eso es normal en una busqueda amplia).

BOGOTA_QUERIES: dict[str, list[str]] = {
    "Hacienda": [
...
def collect_bogota_news(start: datetime, end: datetime, max_workers: int = 8) -> list[NewsItem]:
    """
    Recorre TODOS los temas y frases de busqueda definidos en
    BOGOTA_QUERIES (al inicio del archivo), descarga los resultados de cada
    una, se queda solo con las noticias dentro de la franja horaria pedida,
    y al final elimina los duplicados. Devuelve la lista final de noticias
    de Bogota, ordenada de la mas reciente a la mas antigua.
    """
    # Se arma una lista de tareas: (tema, frase de busqueda) por cada
    # combinacion definida en BOGOTA_QUERIES.
    tasks = [
        (categoria, query)
        for categoria, queries in BOGOTA_QUERIES.items()
...
def main():
    ...
    Esta es la funcion que organiza todo el proceso, de principio a fin:
      ...
      2) Calcula la franja horaria a usar.
      3) Busca las noticias de Bogota.
      4) Busca las noticias nacionales.
      5) Busca el link real y el resumen de cada noticia encontrada.
      6) Genera el archivo de Excel con todo.
    ...
    # Paso 1: decidir el rango de horas a buscar.
    ...
    # Paso 2: buscar las noticias (Bogota y luego nacionales).
    bogota_items = collect_bogota_news(start, end)
    national_items = collect_national_news(top_n=3)

    # Paso 3: para cada noticia encontrada, buscar su link real y su resumen.
    enrich_items(bogota_items + national_items)

    # Paso 4: guardar todo en el archivo de Excel.
